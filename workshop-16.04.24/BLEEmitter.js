...
function motionEvent(BTHparsed) {
    print("Motion event detected, emitting event");
    Shelly.emitEvent("BLU_MOTION", {
        addr: BTHparsed.addr,
        rssi: BTHparsed.rssi,
        Button: BTHparsed.Button,
        Battery: BTHparsed.Battery,
        Illuminance: BTHparsed.Illuminance,
        Motion: BTHparsed.Motion
    });
}
...
let CONFIG = {
    shelly_blu_name_prefixes: ["SBMO", "SBBT"],
    //shelly_blu_address: "bc:02:6e:c3:c8:b9",
    // 38:39:8f:ee:ba:57
    actions: [
        {
            cond: {
                addr: "38:39:8f:a5:9d:19"
            },
            action: motionEvent,
        },
        {
            cond: {
                addr: "38:39:8f:ee:ba:57"
            },
            action: motionEvent,
        },
        {
...
let ACTIVE_SCAN =
    typeof CONFIG.shelly_blu_name_prefixes !== "undefined" &&
    CONFIG.shelly_blu_name_prefixes !== null;
...
function scanCB(ev, res) {
    ...
    if (typeof res.local_name === "undefined")
        return;
    let found = false;
    for (name_prefix_idx in CONFIG.shelly_blu_name_prefixes) {
        if (res.local_name.indexOf(CONFIG.shelly_blu_name_prefixes[name_prefix_idx]) === 0) {
            found = true;
            break;
        }
    }
